    ...
    handleEditZone = () => {
        this.setState({ zone: store.record.zone }, () => {
            Modal.confirm({
                ...
                onOk: () => http.patch('/api/v1/host', { id: store.record.id, zone: this.state.editZone })
                    .then(res => {
        ...
        return (
            <Modal
                ...
                <Form labelCol={{ span: 6 }} wrapperCol={{ span: 14 }}>
                    <Form.Item required label="主机类别">
                        <Col span={14}>
                            {getFieldDecorator('zone', { initialValue: info['zone'], rules: [{ required: true, message: '请输入主机类别/区域/分组' }] })(
                    ...
                    <Form.Item required label="主机名称">
                        {getFieldDecorator('name', { initialValue: info['name'], rules: [{ required: true, message: '请输入主机名称' }]  })(
                            <Input placeholder="请输入主机名称" />
                        )}
                    </Form.Item>
                    <Form.Item required label="连接地址" style={{ marginBottom: 0 }}>
                        <Form.Item style={{ display: 'inline-block', width: 'calc(30%)' }}>
                            {getFieldDecorator('username', { rules: [{ required: true, message: '请输入用户名' }], initialValue: info['username'] })(
                                <Input addonBefore="ssh" placeholder="用户名" />
                            )}
                        </Form.Item>
                        <Form.Item style={{ display: 'inline-block', width: 'calc(40%)' }}>
                            {getFieldDecorator('hostname', { rules: [{ required: true, message: '请输入主机名/ip' }],initialValue: info['hostname'] })(
                                <Input addonBefore="@" placeholder="主机名/IP" />
                            )}
                        </Form.Item>
                        <Form.Item style={{ display: 'inline-block', width: 'calc(30%)' }}>
                            {getFieldDecorator('port', { rules: [{ required: true, message: '请输入主机端口' }],initialValue: info['port'] })(
